    ...
    <main className="min-h-[70vh] bg-[var(--milk-sage)] px-5 md:px-8 py-10 md:py-14">
      <div className="max-w-7xl mx-auto">
        ...
        {tab === "orders" ? (
          ...
          <ProductsSection products={filteredProducts} search={search} setSearch={setSearch} saving={saving} onSave={saveStock} />
        )}
      </div>
    </main>
  );
...
function ProductsSection({ products, search, setSearch, saving, onSave }: { products: Product[]; search: string; setSearch: (v: string) => void; saving: string | null; onSave: (id: string, stock: number) => void }) {
  return (
    <div>
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-5 mb-5"><div><h2 className="font-display text-2xl text-[var(--deep-wine)]">Products & Inventory</h2><p className="text-sm text-[var(--muted)] mt-1">Update stock directly from your admin panel.</p></div><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search products…" className="w-full md:w-72 border border-[var(--line)] bg-white rounded-full px-5 py-3 text-sm outline-none" /></div>
