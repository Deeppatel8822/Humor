    ...
    <div className="max-w-7xl mx-auto px-5 md:px-8 py-14">
      ...
      <div className="grid md:grid-cols-2 gap-12 mb-20">
          ...
          <div className="flex md:flex-col gap-3 overflow-x-auto md:overflow-visible">
            {gallery.map((image, index) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className={`block shrink-0 aspect-square overflow-hidden rounded-xl border ${index === 0 ? "border-[var(--warm-gold)]" : "border-[var(--line)]"}`}
              >
                <img src={image.src} alt={image.alt} className="w-full h-full object-cover" />
              </a>
            ))}
          </div>
          <div className="aspect-square rounded-3xl overflow-hidden bg-[var(--milk-sage)]">
            {gallery.length > 0 ? (
              <img
                src={gallery[0].src}
                alt={gallery[0].alt}
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mb-6">
            <span className="text-2xl font-medium text-[var(--deep-wine)]">&#8377;{product.price_inr}</span>
            {onSale && (
              <span className="text-base text-[var(--ink)]/40 line-through">&#8377;{product.compare_at_price_inr}</span>
            )}
          </div>
        ...
        <div>
          ...
          <ul className="space-y-3">
            {product.key_benefits.map((b) => (
              <li key={b} className="flex gap-3 text-sm text-[var(--ink)]/85"><span className="text-[var(--warm-gold)] mt-0.5">&#10003;</span>{b}</li>
            ))}
          </ul>
          <h2 className="font-display text-2xl text-[var(--deep-wine)] mt-10 mb-2">{product.category === "haircare" ? "Suitable for" : "Skin type"}</h2>
          <div className="flex flex-wrap gap-2">
            {product.skin_hair_type.map((t) => <span key={t} className="text-xs px-3 py-1.5 rounded-full bg-[var(--milk-sage)] text-[var(--deep-wine)]">{t}</span>)}
          </div>
        </div>
        ...
        <div>
          ...
          {product.full_ingredient_list && (
            <details className="mt-6 text-xs text-[var(--muted)]">
              <summary className="cursor-pointer text-[var(--deep-wine)] font-medium">Full ingredient list</summary>
      ...
      {product.how_to_use && (
        <div className="mb-20 bg-[var(--milk-sage)] border border-[var(--line)] rounded-3xl p-8 md:p-10">
          ...
          <p className="text-sm text-[var(--ink)]/85 leading-relaxed max-w-2xl">{product.how_to_use}</p>
        </div>
      )}
      ...
      <div className="mb-20">
        <div className="flex items-baseline justify-between mb-6">
          ...
          <span className="text-xs text-[var(--ink)]/50">* Live reviews pull from the `reviews` table once connected</span>
        </div>
        <div className="bg-[var(--milk-sage)] border border-[var(--line)] rounded-2xl p-8 text-center text-sm text-[var(--muted)]">No reviews yet for this product — be the first to leave one after your order arrives.</div>
      </div>

      <div className="mb-20 max-w-3xl">
        <h2 className="font-display text-2xl text-[var(--deep-wine)] mb-6">Frequently asked questions</h2>
        <div className="space-y-3">
          {genericFaqs.map((f) => (
            <details key={f.q} className="bg-[var(--milk-sage)] border border-[var(--line)] rounded-xl p-5">
              <summary className="cursor-pointer text-sm font-medium text-[var(--deep-wine)]">{f.q}</summary>
      ...
      {routinePartner && (
        <div className="mb-20 bg-[var(--milk-sage)] border border-[var(--line)] rounded-3xl p-8 md:p-10">
          <h2 className="font-display text-2xl text-[var(--deep-wine)] mb-6">Complete your {product.subrange} routine</h2>
      ...
      {related.length > 0 && (
        <div>
          <h2 className="font-display text-2xl text-[var(--deep-wine)] mb-6">You may also like</h2>
