...
export default function AdminProfileWidget() {
  ...
  return (
    <div ref={ref} className="fixed top-5 right-5 z-[100]">
      <button type="button" onClick={() => setOpen((value) => !value)} className="flex items-center gap-2 rounded-full border border-[var(--line)] bg-white px-4 py-2.5 text-xs font-medium text-[var(--deep-wine)] shadow-sm hover:shadow-md transition" aria-expanded={open}>
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-[360px] max-w-[calc(100vw-2rem)] rounded-2xl border border-[var(--line)] bg-white p-5 shadow-xl">
          ...
          <div className="border-t border-[var(--line)] pt-4">
            <p className="text-xs leading-5 text-[var(--muted)]">Password login is disabled. Every admin login is verified with a one-time OTP.</p>
            <button type="button" onClick={logout} className="mt-4 w-full rounded-full border border-red-200 bg-white py-2.5 text-sm font-medium text-red-600 hover:bg-red-50">Logout</button>
          </div>
        </div>
      )}
    </div>
